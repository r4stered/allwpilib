...
TEST_CASE("SpectrumTest RejectsTooFewSamples", "[filterdesigner]") {
  ...
  CHECK_FALSE(Spectrum::Compute(one, 1000.0).has_value());
  std::vector<double> empty;
  CHECK_FALSE(Spectrum::Compute(empty, 1000.0).has_value());
}

TEST_CASE("SpectrumTest RejectsNonPositiveSampleRate", "[filterdesigner]") {
  std::vector<double> s(16, 0.0);
  CHECK_FALSE(Spectrum::Compute(s, 0.0).has_value());
  CHECK_FALSE(Spectrum::Compute(s, -1.0).has_value());
}
